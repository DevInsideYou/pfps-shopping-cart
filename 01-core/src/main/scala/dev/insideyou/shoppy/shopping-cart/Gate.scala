...
trait Gate[F[_]] extends HasConfig[F, Config] with Storage[F] with Redis[F]

object Gate {
  def make[F[_]](hasConfig: HasConfig[F, Config], storage: Storage[F], redis: Redis[F]): Gate[F] =
    new Gate[F] {
      override def config: F[Config] =
        hasConfig.config

      override def findItemsById(itemId: items.ItemId): F[Option[items.Item]] =
        storage.findItemsById(itemId)
